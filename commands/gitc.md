---
description: Commit the current changes with a structured, emoji-tagged message
argument-hint: "[language]"
---

# /gitc — Structured commit

Commit all current changes (staged + unstaged) with a clean, structured message.
...
2. If there is nothing to commit, stop and tell the user.
...
6. If a pre-commit hook fails, fix the underlying issue, re-stage, and create a **new** commit (never `--amend`).

7. After the commit, run `git status` and report the new HEAD short SHA + subject to the user.
